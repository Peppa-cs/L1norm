...
int picwidth = 32;
int picheight = 32;
string NetworkName = "vgg16cifar-l1norm";

/*
vgg16cifar10baseline
resnet56cifar10baseline
renet50cifar10baseline
vgg16cifar-l1norm
resnet56cifar-l1norm A / B
vgg16cifar-soft
resnet56cifar-soft
resnet50cifar-soft
*/
...
ICudaEngine* networkToEngine(const BuildOptions& build, const SystemOptions& sys, IBuilder& builder, INetworkDefinition& network, std::ostream& err)
{
   ...
   */ 
    if (build.int8 || int8IO)
    {
        ...
        config->setInt8Calibrator(new Int8EntropyCalibrator2<BINcifarBatchStream>( calibrationStream, -1,  build.network.c_str(),
