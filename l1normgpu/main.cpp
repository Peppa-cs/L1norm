...
/
int batcharray[] = {1,32,64,128};

for(int i = 0 ; i < 4 ; i++){

    int inferencebatch = batcharray[i];

    void* buffers[2];

    float* prob = new float[inferencebatch * numclass];
    int count=0;
    ...
    string name0 = "/home/peppa1/resultsgpu/" + options.build.network + precision + "h2dtime_batch-" +device+ ".txt";
    string name1 = "/home/peppa1/resultsgpu/" + options.build.network + precision + "comptime_batch-"+device + ".txt";
    string name2 = "/home/peppa1/resultsgpu/" + options.build.network + precision + "d2htime_batch-" +device + ".txt";

    ofstream outputfile0( name0 ,ios::app);
    ofstream outputfile1( name1 ,ios::app);
    ofstream outputfile2( name2 ,ios::app);

    outputfile0<< "      batchsize is     "<<inferencebatch<<std::endl;
    outputfile1<< "      batchsize is     "<<inferencebatch<<std::endl;
    outputfile2<< "      batchsize is     "<<inferencebatch<<std::endl;
    
    CHECK(cudaMalloc((void**)&buffers[0], inferencebatch*3*picwidth*picheight*sizeof(float)));
    CHECK(cudaMalloc((void**)&buffers[1], inferencebatch*numclass*sizeof(float)));
  ...
  //  auto InputDims = context->getBindingDimensions(0);
    BINcifarBatchStream pStream( inferencebatch,iteration,InputDims);
	...
	cudaEvent_t start0, end0;
     ...
     //  int buffsize = mParams.int8 ? mParams.batchSize*3*picwidth*picheight*sizeof(float):mParams.batchSize*3*picwidth*picheight*sizeof(float)/2;
        int buffsize = inferencebatch*3*picwidth*picheight*sizeof(float);  
	...
	cudaEventRecord(end0, 0);
        ...
        bool status = context->enqueue(inferencebatch, buffers, stream, nullptr);
        if (!status)
        {
            return false;
        }
	...
	cudaEvent_t start2, end2;
        ...
        CHECK(cudaMemcpy(prob,buffers[1],inferencebatch*10*sizeof(float), cudaMemcpyDeviceToHost));
	...
	cudaEventRecord(end2, 0);
        ...
        /*if(!verifyOutput(prob,count,pStream.getLabel(),inferencebatch))
        {
